...
spark = SparkSession.builder.appName("sthupakula p2 ").config('spark.driver.host','spark-edge.service.consul').config(conf=conf).getOrCreate()
...
splitDF.coalesce(1).write.mode("overwrite").option("header","true").csv("s3a://sthupakula/90.csv")
